...
function initializePlayer(firstTrack) {
  globalAP = new APlayer({
    ...
    listmaxheight: '0px',
    music: {
        title: `<a target='_blank' href="${firstTrack.external_urls.spotify}">${firstTrack.name}</a>`,
  ...
  // let volumeBar = document.querySelector('.aplayer-volume-bar-wrap');
  // volumeBar.addEventListener('click', function(e) {globalVolume = globalAP.audio.volume;});
  let current = 0;
...
function run(link, globalAP){
  globalAP.addMusic([{
      title: `<a target='_blank' href="${link.external_urls.spotify}">${link.name}</a>`,
